...
`terraform-provider-opensearch`.  Substitute
...
* `provider/` contains the Go code used to create the provider as well as
  ...
  * `provider/cmd/pulumi-tfgen-opensearch`
    generates the Pulumi resource schema (`schema.json`), based on the Terraform
    provider's resources.
  * `provider/cmd/pulumi-resource-opensearch`
...
1. In `provider/go.mod`, add a reference to the upstream Terraform provider in the `require` section, e.g.
    ...
    ```go
    github.com/foo/terraform-provider-opensearch v0.4.0
    ```

1. In `provider/resources.go`, ensure the reference in the `import` section uses the correct Go module path, e.g.:

    ```go
    github.com/foo/terraform-provider-opensearch/opensearch
    ```
...
1. Create the schema by running the following command:
    ...
    ```text
    warning: resource opensearch_something not found in provider map; skipping
    warning: resource opensearch_something_else not found in provider map; skipping
    warning: data source opensearch_something not found in provider map; skipping
    warning: data source opensearch_something_else not found in provider map; skipping
    ```
...
1. **Add resource mappings:** For each resource in the provider, add an entry in
   the `Resources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "opensearch_something":      {Tok: tfbridge.MakeResource(mainPkg, mainMod, "Something")},
    "opensearch_something_else": {Tok: tfbridge.MakeResource(mainPkg, mainMod, "SomethingElse")},
    ```
...
1. **Add data source mappings:** For each data source in the provider, add an entry in the `DataSources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "opensearch_something":      {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomething")},
    "opensearch_something_else": {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomethingElse")},
    ```
...
1. **Add provider configuration overrides (not typically needed):** Pulumi's
   ...
   `opensearch_API_KEY`.  Examples of common
   uses cases:
    ...
    "apikey": {
        Default: &tfbridge.DefaultInfo{
            EnvVars: []string{"opensearch_API_KEY"},
        },
    ```
...
1. Create a new Pulumi program in the `examples/` directory, e.g.:
    ...
    npm install
    yarn link @piclemx/pulumi-opensearch
    ```
