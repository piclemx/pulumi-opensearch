...
import (
	...
	"github.com/piclemx/pulumi-opensearch/provider/pkg/version"
)

//go:embed cmd/pulumi-resource-opensearch/bridge-metadata.json
...
func convertName(tfname string) (module string, name string) {
	...
	contract.Assertf(tfNameItems[0] == "opensearch", "Invalid snake case name %s. Does not start with opensearch", tfname)
	module = mainMod
...
func makeDataSource(ds string) tokens.ModuleMember {
	...
	return tfbridge.MakeDataSource("opensearch", mod, "get"+name)
}

func makeResource(res string) tokens.Type {
	mod, name := convertName(res)
	return tfbridge.MakeResource("opensearch", mod, name)
}
	...
	prov := tfbridge.ProviderInfo{
		...
		Golang: &tfbridge.GolangInfo{
			ImportBasePath: filepath.Join(
				fmt.Sprintf("github.com/piclemx/pulumi-%[1]s/sdk/", "opensearch"),
